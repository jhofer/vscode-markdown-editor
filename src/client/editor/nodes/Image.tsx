...
const downloadImageNode = async (node) => {
  console.log("downloadImageNode", node);
    ...
    return {
      ...
      parseDOM: [
        ...
        },
        {
          ...
          getAttrs: (dom: HTMLImageElement) => {
            console.log("img.getAttrs", dom);
            return {
              src: dom.getAttribute("rawsrc"),
      ...
      toDOM: (node) => {
        ...
        return [
          ...
          ["img", { ...node.attrs, src, rawsrc, contentEditable: false }],
          ["p", { class: "caption" }, 0],
        ];
      },
    };
  ...
  inputRules({ type }) {
    return [
      new InputRule(IMAGE_INPUT_REGEX, (state, match, start, end) => {
        const [okay, alt, rawsrc, matchedTitle] = match;
        const { tr } = state;
        console.log("inputRules", match);
        if (okay) {
          tr.replaceWith(
            start - 1,
            end,
